<div class="bg-gradient-to-b from-black to-transparent fixed left-0 right-0 top-0 z-50">
    ...
    <div class="flex items-center flex-col lg:flex-row py-8 lg:py-0">
        ...
        <div class="lg:order-2 order-2 w-full xl:w-6/12 lg:w-8/12 container mx-auto lg:px-4 lg:py-12 overflow-x-auto lg:overflow-visible">
            <ul class="flex items-center md:justify-center gap-2 py-4">
                @foreach ($menu as $item)
                    <li>
                        <a
                            href="{{ url(app()->getLocale()) . $item['url'] }}" class=" {{ $active == $item['title'] ? 'text-black bg-white' : 'text-white bg-white/30' }} text-sm rounded-full px-6 py-2 font-semibold uppercase">
                            {{ $item['title'] }}
                        </a>
                    </li>
                @endforeach
        ...
        <div class="lg:order-3 order-1 w-full xl:w-4/12 lg:w-4/12 items-center flex nav-title transition-all duration-500 transform -translate-y-[600%]">
            ...
            <div class="flex items-center justify-center lg:justify-end h-full">
