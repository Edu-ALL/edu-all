<div id="sub-navbar" class="bg-gradient-to-b from-black to-transparent fixed left-0 right-0 top-0 z-50 transition-all duration-300 ease-in-out">
    <nav class="px-4 lg:px-10 xl:px-12 max-w-screen-2xl mx-auto overflow-hidden">
        <div class="flex items-center flex-col lg:flex-row py-8 lg:py-0">
            ...
            <div
                class="lg:order-2 order-2 w-full xl:w-7/12 lg:w-8/12 container mx-auto lg:px-4 lg:py-12 overflow-x-auto lg:overflow-visible">
                <ul class="flex items-center md:justify-center gap-2 py-4">
                    @foreach ($menu as $item)
                        <li>
                            <a href="{{ url(app()->getLocale()) . $item['url'] }}"
                                class=" {{ $active == $item['title'] ? 'text-black bg-white' : 'text-white bg-white/30' }} text-sm rounded-full px-6 py-2 font-semibold uppercase whitespace-nowrap">
