...
@if (isset($isButton) && $isButton == true)
    <div class="md:hidden flex justify-center mt-2 md:z-[50] z-[10]">
        <button class="bg-newprimary py-2 px-4 text-white rounded-lg"
            onclick="open_registration()">{{ $buttonTitle ?? 'Register Now' }}</button>
    </div>
@else
    @if ($isPartner)
    ...
    @else
        <div class="absolute w-full h-full md:max-w-screen-xl mx-auto top-0 left-0 md:-right-[15%] right-0">
