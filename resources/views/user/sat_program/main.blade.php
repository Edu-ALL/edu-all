...
@section('content')
        ...
        <div class="relative flex w-full left-0 overflow-hidden">
            <div class="absolute-center main-container w-full h-full md:flex md:flex-col md:justify-between">
                <div class="flex flex-col items-start pt-10 mb-14 md:pl-28 lg:pt-24">
                    <h1 class="font-newprimary font-black text-4xl text-white uppercase md:text-[42px] md:text-left">
                        {{ __('pages/programs/sat_program.title') }}
                    </h1>
                    <p class="mt-6 font-newprimary font-normal text-xl text-white md:text-left md:w-1/2">
                        {{ __('pages/programs/sat_program.desc') }}</p>
                    <a href="{{ route('sign_me_sat_prep', app()->getLocale()) }}"
                        class="inline-block px-5 py-2 mt-10 font-newprimary font-bold text-lg text-white bg-newyellow rounded-lg">
    ...
    <section class="py-10">
        <div class="flex flex-col items-center main-container md:flex-row lg:gap-x-20">
            ...
            <div class="mt-6 flex flex-col w-full md:w-1/2">
                <h2 class="mb-8 font-newprimary font-black text-[26px] text-newprimary tracking-tighter uppercase md:text-6xl">
                    {{ __('pages/programs/sat_program.subjects_title') }}</h2>
                <ul class="flex flex-col gap-y-2">
                    @foreach (__('pages/programs/sat_program.subjects_list') as $item)
                        <li class="px-4 py-3 rounded-2xl bg-newyellow">
                            <h5 class="font-newprimary font-bold text-xl text-white md:px-3 md:text-2xl">{{ $item }}
                            </h5>
                        </li>
                    @endforeach
                </ul>
                <span class="mt-1 font-newprimary text-base text-black text-right">*ACT Prep Class</span>
            </div>
        </div>
    </section>
                    ...
                    @foreach (__('pages/programs/sat_program.do_list') as $item)
                        <div
                            ...
                            <div class="flex flex-col justify-center w-4/5 mt-2 pr-4 pb-4 py-3 sm:mt-0">
                                <h4 class="font-newprimary font-bold text-2xl text-yellow">
                                    {{ $item['title'] }}</h4>
                                <ul class="list-outside pr-6">
                                    @foreach ($item['body'] as $desc)
                                        <li class="inline-flex text-white">
                                            ...
                                            <p class="font-newprimary font-normal text-[17px] text-white leading-5 md:pr-4">
                                                {{ $desc }}</p>
                                        </li>
                                    @endforeach
                                </ul>
    ...
    <section class="py-10">
        <div class="grid grid-cols-1 gap-x-20 main-container md:grid-cols-2">
            <div class="flex flex-col md:gap-y-2">
                <h2 class="font-newprimary font-black text-3xl text-newprimary tracking-tighter uppercase md:text-5xl">
                    {{ __('pages/programs/sat_program.private_title') }}</h2>
                <span
                    class="font-newprimary font-semibold text-lg text-yellow md:text-2xl">{{ __('pages/programs/sat_program.private_sub') }}</span>
                <p class="font-newprimary font-semibold text-lg text-newprimary leading-5 md:text-2xl">
                ...
                <ul class="flex flex-col gap-y-3 my-4">
                    @foreach (__('pages/programs/sat_program.private_list') as $item)
                        <li class="px-5 py-4 rounded-2xl bg-newprimary">
                            <h4 class="font-newprimary font-bold text-lg text-yellow uppercase">{{ $item['title'] }}</h4>
                            <p class="font-newprimary text-base text-white leading-5">{{ $item['body'] }}</p>
                        </li>
                    @endforeach
                </ul>
            </div>
            <div class="flex flex-col md:gap-y-2">
                ...
                <span
                    class="font-newprimary font-semibold text-lg text-yellow md:text-2xl">{{ __('pages/programs/sat_program.semi_sub') }}</span>
                <p class="font-newprimary font-semibold text-lg text-newprimary leading-5 md:text-2xl">
                        ...
                        <li class="px-5 py-4 rounded-2xl bg-newprimary">
                            ...
                            @if ($item['btn'] != null)
                                <a href="{{ asset('assets/files/sat-program/SAT CURRICULUM.pdf') }}" target="_blank"
                                    class="inline-block mt-3 px-3 py-1.5 font-newprimary font-medium text-base text-white rounded-2xl bg-newyellow md:rounded-xl">{{ $item['btn'] }}</a>
                            @endif
                        </li>
                    @endforeach
                </ul>
            ...
            <div class="text-center px-6 md:col-span-2">
                <a href="{{ route('sign_me_sat_prep', app()->getLocale()) }}"
                    class="inline-flex px-5 py-2 mt-4 font-newprimary font-bold text-base text-white text-center bg-newyellow rounded-2xl  md:text-xl">{{ __('pages/programs/sat_program.class_btn') }}</a>
            </div>
        </div>
    </section>
    ...
    <section class="py-10">
        <div class="main-container flex flex-col items-center">
            ...
            <a href="{{ route('sign_me_sat_prep', app()->getLocale()) }}"
                class="px-5 py-2 mt-4 font-newprimary font-bold text-xl text-white text-center bg-newyellow rounded-2xl">{{ __('pages/programs/sat_program.bottom_btn') }}</a>
        </div>
