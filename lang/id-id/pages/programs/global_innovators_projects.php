<?php

return [
    'description' => 'Lihat dunia sambil membuat perbedaan! <br> Dapatkan pengalaman langsung, kembangkan keterampilan yang berharga, dan buat dampak positif di lingkup lokal atau bahkan global!',
    'description_button' => 'Daftar Sekarang',

    'community_empowerment_program_date' => 'Salatiga ', ' 20 - 27 Juni 2023',
    'community_empowerment_program_desc' => 'Mulailah perjalanan Anda dengan pengalaman langsung di mana Anda dapat mengidentifikasi masalah aktual yang terjadi di komunitas lokal Indonesia dan mengembangkan project yang nyata dan berdampak.',
    'community_empowerment_program_btn' => 'Info Lebih Lanjut',

    'global_immersion_program_date' => 'Singapura | Juli 2023',
    'global_immersion_program_desc' => 'Tingkatkan pengetahuan dan keterampilan Anda ke level selanjutnya untuk komunitas global! Kembangkan keahlian yang berharga di bidang yang Anda minati, dan berikan implementasi kehidupan nyata dengan hasil yang jelas.',
    'global_immersion_program_btn' => 'Info Lebih Lanjut',

    'science_research_physical_science_title' => 'PHYSICAL SCIENCE',
    'science_research_physical_science_date' => '2 - 7 Juli 2023',
    'science_research_physical_science_point' => [
        'Robotika ', ' Energi Terbarukan ', ' Environmental Sustainability'
    ],
    'science_research_life_science_title' => 'LIFE SCIENCE',
    'science_research_life_science_date' => '5 - 10 Juli 2023',
    'science_research_life_science_point' => [
        'Bioteknologi ', ' Bioinformatika ', ' Ilmu Forensik DNA ', ' Rekayasa Genetika'
    ],

    'business_date' => '10-14 Juli 2023',
    'business_desc' => 'Ubah project Anda menjadi bisnis yang digerakkan oleh tujuan dan kuasai seluruh proses bisnis. Program ini didasarkan pada pengalaman dan keterampilan yang diperoleh dari Community Empowerment Program dan Science Research Immersion Program, memungkinkan Anda membawa pengaruh Anda ke level yang lebih tinggi.',
    'business_point' => [
        'Identifikasi Masalah ', ' Pembuatan Ide ', ' Pembuatan Prototype ', ' Pitching ', ' Penemuan Customer'
    ],

    'form_title' => 'DAFTAR SEKARANG!',
    'form_desc' => "Karena keterbatasan jumlah peserta yang tersedia, kami menyarankan Anda untuk mengamankan pendaftaran Anda sekarang. <br/> Pendaftaran program kami cenderung berlangsung cepat, jadi jangan lewatkan kesempatan Anda!"
];
