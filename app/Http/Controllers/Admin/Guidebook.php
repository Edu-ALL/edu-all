        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                ...
                ->editColumn('image', function ($d) {
                    $path = Storage::url('guidebook/' . $d->created_at->format('Y') . '/' . $d->created_at->format('m') . '/' . $d->guidebook_image);
                    $result = '
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('guidebook_image_en')) {
                ...
                $destinationPath_en = 'project/eduall-website/guidebook/' . date('Y') . '/' . date('m') . '/';
            ...
            if ($request->hasFile('guidebook_image_id')) {
                ...
                $destinationPath_id = 'project/eduall-website/guidebook/' . date('Y') . '/' . date('m') . '/';
        ...
        try {
            ...
            if ($request->hasFile('guidebook_image_en')) {
                if ($old_image_path_en = $guidebook_en->guidebook_image) {
                    $file_path = 'project/eduall-website/mentor/' . $guidebook_en->created_at->format('Y') . '/' . $guidebook_en->created_at->format('m') . '/' . $old_image_path_en;
                ...
                $file_en = $request->file('guidebook_image_en');
                $file_format_en = $request->file('guidebook_image_en')->getClientOriginalExtension();
                $destinationPath_en = 'project/eduall-website/guidebook/' . $guidebook_en->created_at->format('Y') . '/' . $guidebook_en->created_at->format('m') . '/';
                $time = $guidebook_en->group;
                $fileName_en = 'Guidebook-en-' . $time . '.' . $file_format_en;
                Storage::disk('s3')->put($destinationPath_en . $fileName_en, file_get_contents($file_en));
                $guidebook_en->guidebook_image = $fileName_en;
            ...
            if ($request->hasFile('guidebook_image_id')) {
                if ($old_image_path_id = $guidebook_id->guidebook_image) {
                    $file_path = 'project/eduall-website/mentor/' . $guidebook_id->created_at->format('Y') . '/' . $guidebook_id->created_at->format('m') . '/' . $old_image_path_id;
                    if (Storage::disk('s3')->exists($file_path)) {
                        Storage::disk('s3')->delete($file_path);
                    }
                }
                $file_id = $request->file('guidebook_image_id');
                $file_format_id = $request->file('guidebook_image_id')->getClientOriginalExtension();
                $destinationPath_id = 'project/eduall-website/guidebook/' . $guidebook_id->created_at->format('Y') . '/' . $guidebook_id->created_at->format('m') . '/';
                $time = $guidebook_id->group;
                $fileName_id = 'Guidebook-id-' . $time . '.' . $file_format_id;
                Storage::disk('s3')->put($destinationPath_id . $fileName_id, file_get_contents($file_id));
                $guidebook_id->guidebook_image = $fileName_id;
    ...
    public function delete($group)
    {
        ...
        try {
            ...
            if ($old_image_path_en = $guidebook[0]->guidebook_image) {
                $file_path_en = 'project/eduall-website/guidebook/' . $guidebook[0]->created_at->format('Y') . '/' . $guidebook[0]->created_at->format('m') . '/' . $old_image_path_en;
            ...
            if ($old_image_path_id = $guidebook[1]->guidebook_image) {
                $file_path_id = 'project/eduall-website/guidebook/' . $guidebook[1]->created_at->format('Y') . '/' . $guidebook[1]->created_at->format('m') . '/' . $old_image_path_id;
