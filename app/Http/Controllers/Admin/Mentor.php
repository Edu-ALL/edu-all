...
class Mentor extends Controller
{
    ...
    public function getMentorValues(Request $request, $group)
    {
        if ($request->ajax()) {
            $data = MentorValues::where('mentor_id', $group)->orderBy('updated_at', 'desc')->get();
            return Datatables::of($data)
                ->addIndexColumn()
                ->editColumn('lang', function ($d) {
                    $path = asset('assets/img/flag/flag-' . $d->lang . '.png');
                    $result = '
                    <img data-original="' . $path . '" src="' . $path . '" alt="" width="30">
                ';
